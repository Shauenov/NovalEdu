...
from app.core.constants import ROLE_ADMIN, ROLE_CONDUCTOR
...
class AlumniService:
    ...
    async def get_story(self, story_id: UUID, requester_role: str):
        ...
        if not story.is_published and requester_role not in (ROLE_ADMIN, ROLE_CONDUCTOR):
            raise NotFoundException("Story not found")
        return story
    ...
    async def create_story(self, data: AlumniCreate, author_id: UUID, requester_role: str):
        if requester_role not in (ROLE_ADMIN, ROLE_CONDUCTOR):
            raise ForbiddenException("Only conductor or admin can create alumni stories")
    ...
    async def update_story(self, story_id: UUID, data: AlumniUpdate, requester_role: str):
        if requester_role not in (ROLE_ADMIN, ROLE_CONDUCTOR):
            raise ForbiddenException("Only conductor or admin can update alumni stories")
        story = await self.repo.get_by_id(story_id)
        if not story:
            raise NotFoundException("Story not found")
    ...
    async def delete_story(self, story_id: UUID, requester_role: str) -> None:
        if requester_role not in (ROLE_ADMIN, ROLE_CONDUCTOR):
            raise ForbiddenException("Only conductor or admin can delete alumni stories")
        story = await self.repo.get_by_id(story_id)
        if not story:
            raise NotFoundException("Story not found")
